  ...
  float ledBrightness1 = 0;
  float ledBrightness2 = 0;
  float ledBrightness3 = 0;       // 🆕 3번 LED 밝기
  float ledBrightness4 = 0;       // 🆕 4번 LED 밝기
  ...
  float strawberry_target_photo = 540;
  float tomato_target_photo = 540;
  ...
  float target_stop_hum = target_hum - 10;
  ...
  void loop() {
      ...
      // 대시보드 데이터 수신 (ID+TYPE+VALUE)
        ...
        if (id_str != NULL && type_str != NULL && val1_str != NULL) {
          ...
          // ✨ Type이 0(센서)일 때 수신된 값을 목표값으로 덮어씌움
            if (rx_type == 1) {
              switch (rx_id) {
                case 1: case 2: // 딸기 조도 목표값 변경
                  strawberry_target_photo = rx_value1; // rx_value -> rx_value1으로 변경
                  if (strawberry_target_photo <= D) {
                    strawberry_target_photo = D + 1;
                  }
                  break;
                
                case 3: case 4: // 토마토 조도 목표값 변경
                  tomato_target_photo = rx_value1;
                  if (tomato_target_photo <= D) {
                    tomato_target_photo = D + 1;
                  }
                  break;
    ...
    if (currentMillis - previousMillis >= main_interrupt) {
      ...
      Serial1.print("1+"); Serial1.print("0+"); Serial1.println(lightPercent1);
      Serial1.print("1+"); Serial1.print("1+"); Serial1.println(ledBrightness1 / 255.0, 2);
      
      Serial1.print("2+"); Serial1.print("0+"); Serial1.println(lightPercent2);
      Serial1.print("2+"); Serial1.print("1+"); Serial1.println(ledBrightness2 / 255.0, 2);
      
      Serial1.print("3+"); Serial1.print("0+"); Serial1.println(lightPercent3);
      Serial1.print("3+"); Serial1.print("1+"); Serial1.println(ledBrightness3 / 255.0, 2);
      
      Serial1.print("4+"); Serial1.print("0+"); Serial1.println(lightPercent4);
      ...
      Serial1.print("딸기 목표 조도"); Serial1.print("0+"); Serial1.println(strawberry_target_photo);
      Serial1.print("토마토 목표 조도"); Serial1.print("0+"); Serial1.println(tomato_target_photo);
      Serial1.print("딸기 목표 토양"); Serial1.print("0+"); Serial1.println(strawberry_target_soil);
      Serial1.print("토마토 목표 토양"); Serial1.print("0+"); Serial1.println(tomato_target_soil);
      Serial1.print("목표 온도"); Serial1.print("0+"); Serial1.println(target_tem);
      Serial1.print("목표 습도"); Serial1.print("0+"); Serial1.println(target_hum);
    }
    ...
    if (currentMillis - prevLoopMillis >= LOOP_INTERVAL) {
      ...
      ledBrightness1 = constrain(map(filtered_photo1, D, strawberry_target_photo, 255, 0), 0, 255);
      filtered_photo2 = (filtered_photo2 * 0.9) + (photo2 * 0.1);
      ledBrightness2 = constrain(map(filtered_photo2, D, strawberry_target_photo, 255, 0), 0, 255);
      filtered_photo3 = (filtered_photo3 * 0.9) + (photo3 * 0.1); // 🆕
      ledBrightness3 = constrain(map(filtered_photo3, D, tomato_target_photo, 255, 0), 0, 255);
      filtered_photo4 = (filtered_photo4 * 0.9) + (photo4 * 0.1); // 🆕
      ledBrightness4 = constrain(map(filtered_photo4, D, tomato_target_photo, 255, 0), 0, 255);

      lightPercent1 = constrain(map(filtered_photo1, D, strawberry_target_photo  , 0, 100), 0, 100) / 100.0;
      lightPercent2 = constrain(map(filtered_photo2, D, strawberry_target_photo, 0, 100), 0, 100) / 100.0;
      lightPercent3 = constrain(map(filtered_photo3, D, tomato_target_photo, 0, 100), 0, 100) / 100.0;
      lightPercent4 = constrain(map(filtered_photo4, D, tomato_target_photo, 0, 100), 0, 100) / 100.0;
      ...
      if (fire_state == 0) {
        ...
        if (hum >= target_hum || tem >= target_tem) {
          motor_state = 1;
        } 
        // 습도 60% 이하 및 온도 22도 이하로 모두 떨어졌을 때만 중지
        else if (hum <= target_stop_hum && tem <= target_stop_tem) {
          motor_state = 0;
        }

        if (motor_state == 1) {
          // 습도 오차 비율 (65~75 구간을 0.0 ~ 1.0으로 매핑)
          float hum_ratio = (hum - target_hum) / (75.0 - target_hum);
          hum_ratio = constrain(hum_ratio, 0.0, 1.0);

          // 온도 오차 비율 (25~30 구간을 0.0 ~ 1.0으로 매핑)
          float tem_ratio = (tem - target_tem) / (30.0 - target_tem);
      ...
      Serial.println("]");
      Serial.print("딸기 목표조도");    Serial.println(strawberry_target_photo);
      Serial.print("토마토 목표조도");    Serial.println(tomato_target_photo);
