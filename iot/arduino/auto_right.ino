...
// 보광등 역할용 조도값 쓰레쉬홀드값
const int D1 = 100, BRIGHT_1 = 350; // 조도1은 범위가 낮으므로 조정
const int D2 = 220, BRIGHT_2 = 540;
const int D3 = 210, BRIGHT_3 = 550;
const int D4 = 210, BRIGHT_4 = 570;

// 온습도 센서 관련 변수 선언
float tem = 0.0;
float hum = 0.0;
const float targethum = 65;
const float targettem = 55;
...
void loop() {
  ...
  float soil_revalue = map(soil, 250, 680, 0, 1000);
  soil_revalue = constrain(soil_revalue, 0, 1000) / 1000.0;
  ...
  if (currentMillis - previousMillis >= main_interrupt) {
    ...
    Serial1.print("soil1"); Serial1.println(soil);
    Serial1.print("soil2"); Serial1.println(soil2);
    Serial1.print("soil3"); Serial1.println(soil3);
    Serial1.print("soil4"); Serial1.println(soil4);

    Serial1.print("7+0+"); Serial1.println(soil_revalue);
    Serial1.print("6+1+"); Serial1.println(ledBrightness1 * (0.99 / 255.0));
    Serial1.print("5+1+"); Serial1.println(pump_state == 0 ? 1 : 0);
    Serial1.print("4+1+"); Serial1.println(moto_state);
    Serial1.print("3+0+"); Serial1.println(String(fire_state));
    
    if (!isnan(tem)) { Serial1.print("2+0+"); Serial1.println(String(tem)); }
    if (!isnan(hum)) { Serial1.print("1+0+"); Serial1.println(String(hum)); }
    if (!isnan(filtered_photo1)) { Serial1.print("0+0+"); Serial1.println(String(filtered_photo1)); }
  }
  ...
  if (currentMillis - prevLoopMillis >= LOOP_INTERVAL) {
    ...
    ledBrightness1 = constrain(map(filtered_photo1, D1, BRIGHT_1, 255, 0), 0, 255);
    filtered_photo2 = (filtered_photo2 * 0.9) + (photo2 * 0.1);
    ledBrightness2 = constrain(map(filtered_photo2, D2, BRIGHT_2, 255, 0), 0, 255);
    filtered_photo3 = (filtered_photo3 * 0.9) + (photo3 * 0.1); // 🆕
    ledBrightness3 = constrain(map(filtered_photo3, D3, BRIGHT_3, 255, 0), 0, 255);
    filtered_photo4 = (filtered_photo4 * 0.9) + (photo4 * 0.1); // 🆕
    ledBrightness4 = constrain(map(filtered_photo4, D4, BRIGHT_4, 255, 0), 0, 255);
    ...
    if (fire_state == 0) {
      ...
      if (!is_pumping1) {
        if ((soil_revalue >= 0.6 || soil2_revalue >= 0.6) && (currentMillis - pump1_start_time >= PUMP_COOLDOWN)) {
          pump_state = 0; // 물 주기 (ON)
          digitalWrite(relayPin, pump_state);
          is_pumping1 = true;
    ...
    Serial.print(", P1:");       Serial.print(photo1);
    Serial.print(", P2:");       Serial.print(photo2);
    Serial.print(", P3:");       Serial.print(photo3);
    Serial.print(", P4:");       Serial.print(photo4);
    ...
    Serial.print(", Motor:");    Serial.print(moto_state);
    Serial.print(", Soil1:");    Serial.print(soil_revalue);
    ...
    Serial.print(", Pump1:");    Serial.print(pump_state == 0 ? "ON" : "OFF");
