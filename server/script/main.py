...
def plants():
    ...
    history_records, _ = db.get_plant_statistics(type_ids=type_ids, n=per_page + 1, offset=offset)
        ...
        for i, type_id in enumerate(type_ids):
            ...
            for label in labels:
                if label in records_by_date:
                    ...
                    avg_mat.append(round(sum(r.avg_maturity for r in day_recs) / len(day_recs), 3))
                    dis_rat.append(round(sum(r.disease_ratio for r in day_recs) / len(day_recs), 3))
                else:
                    avg_mat.append(None)
                    dis_rat.append(None)
